...
. "C:\Migration\Scripts\ArchiveData.ps1" -Operation EntraIDUserSignInData -AgeInDays 20
...
$tenants = @(switch ($TenantType)
{
    'Source'
    {
        @{
            ...
            ToProcess = $MigrationList.where({-not [string]::IsNullOrWhiteSpace($_.SourceEntraID)})
        }
    }
    'Target'
    {
        @{
            ...
            ToProcess = $MigrationList.where({-not [string]::IsNullOrWhiteSpace($_.TargetEntraID)})
        }
    }
})
        ...
        $ToProcess = $t.ToProcess
            ...
            try {
                Start-Job -Name "Export $TenantDomain User Registration Detail" -ErrorAction Stop -ScriptBlock { 
                    ...
                    $filter = 
                    Get-MgAuditLogSignIn -  | Select-Object -Property $properties
                }
            }
            catch {
